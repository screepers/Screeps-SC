...
chrome.tabs.onUpdated.addListener(function (tabId, changeInfo, tab) {
  if (changeInfo.status == "complete") {
    // TODO: better support for steamless client
    console.log("tabs.onUpdated", tab.url);

    if (
      tab.url.startsWith("https://screeps.com/a/#!/") ||
      tab.url.startsWith("http://localhost:8080/(http://localhost:21025)")
    ) {
      /*
  ...
  if (activeTabPorts[tabId][info.path].port) {
    ...
    if (injectQueue.length === 0) {
      ...
      chrome.tabs.executeScript(
        tabId,
        { code: `var module = {name: '${info.path}', config: ${JSON.stringify(config)}}` },
        function () {
