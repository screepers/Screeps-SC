module.exports.init = function () {
    ...
    module.ajaxGet("https://screeps.com/api/user/rooms?id=" + userid, function (data, error) {
        ...
        for (const [shardName, shard] of Object.entries(module.exports.shards)) { // return self.req('GET', 'https://screeps.com/api/game/world-size', { shard })
    ...
    style.innerHTML += ".loadButton {place-items: center;margin: 0 20px;border: none;background: transparent;color: #4A5FD2;font-size: 12px;font-weight: 600;line-height: 26px;text-transform: uppercase;}";
    style.innerHTML += "._success {color: #80D47B;}";
    style.innerHTML += "._fail {color: #D2554A;}";
    style.innerHTML += "._number {text-align:right;}";
    style.innerHTML += ".type {display:inline-block; vertical-align: middle; width: 25px;min-height: 37px;text-align: center;background-repeat: no-repeat;}";
    ...
    module.exports.loadNewerButton.onclick = () => { // TODO: handle an issue where you wait for so long that page 0..N actually contains new orders
    ...
    module.exports.loadMoreButton.onclick = () => { // TODO: move focus to new orders
        module.exports.fetchMarketHistoryPage(++ module.exports.page);
    };
...
module.exports.generateHistoryHtmlRow = function (history) { // console.log(history);
    ...
    try {
       ...
       */
            if (history.market.extendOrder) {
                ...
                descriptionCell.innerHTML = `Extend ${
                    module.exports.nFormatter(market.addAmount)
                } ${infoCircle}`;
            } else if (history.market.changeOrderPrice) {
                var market = history.market.changeOrderPrice;
                var infoCircle = '<div class="fa fa-question-circle" title=\'' + JSON.stringify(market) + "'></div>";

                descriptionCell.innerHTML = `Change Price ${
                    module.exports.nFormatter(market.oldPrice)
                } -> ${
                    module.exports.nFormatter(market.newPrice)
                } ${infoCircle}`;
            } else {
                ...
                const amount = market.remainingAmount ? `${
                    module.exports.nFormatter(market.remainingAmount)
                } remaining` : `${
                    module.exports.nFormatter(market.totalAmount)
                } total`;
            ...
            var transactionCost = accountResource ? "" : module.exports.calcTransactionCost(shard, market.amount, roomName, targetRoomName);
            ...
            var infoCircle = '<div class="fa fa-question-circle" title=\'' + JSON.stringify(market) + "'></div>";
            var transactionCostHtml = `(<span style="color:#ff8f8f;margin-right:-12px">-${
                module.exports.nFormatter(transactionCost)
            } ${resourceEnergy}</span>)`;
            ...
            if (accountResource) { // TODO: acquire player, don't think we have that info
        ...
        console.error(error);
        descriptionCell.innerHTML = `Error: ${
            error.message
        } ${infoCircle}`;
    }
    return row;
};
module.exports.resourceImageLink = function (shard, type) { // market-resource--battery has -10px important margin, we need to override that
    return type ? `<a href="#!/market/all/${shard}/${type}">
